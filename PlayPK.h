...
struct PlayerPK {
    ...
    PlayerPK(int hp = 0, int dame = 0, int skillframe = 0, int attackframe = 0, std::string path = "", std::string skillpath = "", SDL_Rect chose = { 0,0,0,0 }) :
        HP(hp), Dame(dame), skill_frame(skillframe), attack_frame(attackframe), Path(path), skill_path(skillpath), choose(chose) {}
};
...
private: 
    ...
    bool p1_IsSkill; int p1_skill_frame, p1_attack_frame;
    float p1_time_skill;
...
private:
    ...
    bool p2_IsSkill; int p2_skill_frame, p2_attack_frame;
    float p2_time_skill;
